...
for(let i=0;i<config.layers.length;i++){
  ...
  const {splited, size, comment} = (()=>{
    ...
    if("raw" in layer){
      ...
      const absolute_path = path.join(DIRNAME,layer.file);
...
let input_comment;
if ("shape" in config.input){
  shape=config.input.shape;
  if (!("start_position" in shape)){
    console.error(`[Error] (Creating cfg.rs: input.shape) "start_position" is not found in input.shape`);
    process.exit(3);
  }

  shape.start_position.x = parseInt(shape.start_position.x);
  if (isNaN(shape.start_position.x)){
    console.error(`[Error] (Creating cfg.rs: input.shape.start_position) "x" must be a number.`);
    process.exit(3);
  }
  shape.start_position.y = parseInt(shape.start_position.y);
  if (isNaN(shape.start_position.y)){
    console.error(`[Error] (Creating cfg.rs: input.shape.start_position) "y" must be a number`);
    process.exit(3);
  }

  if (("password_char" in shape) && shape.password_char.length!==1) {
    console.error(`[Error] (Creating cfg.rs: input.shape) "password_char" must be a char`);
    process.exit(3);
  }

  if("raw" in shape){
    input_comment = "raw";
  }else if("file" in shape){
    
    const absolute_path = path.join(DIRNAME,shape.file);
    input_comment = `file: ${shape.file}`;
    
    if(!fs.existsSync(absolute_path)){
      console.error(`[Error] (Creating cfg.rs: input.shape) ${shape.file} do not exists.`);
      process.exit(4);
    }

    if(!fs.statSync(absolute_path).isFile()){
      console.error(`[Error] (Creating cfg.rs: input.shape) ${shape.file} is not a file.`);
      process.exit(4);
    }

    shape.raw = fs.readFileSync(absolute_path).toString();

  }else{
    console.error(`[Error] (Creating cfg.rs: input.shape) neither 'raw' nor 'file' is found in input.shape.`);
    process.exit(3);
  }
}
let shape_splited = shape.raw.split("\n").filter(str=>str.length!==0);
const shape_length = getAnsiLength(shape_splited[0]);
if (shape_splited.some(str=>getAnsiLength(str)!==shape_length)){
  console.error("[Error] (Creating cfg.rs: input.shape) ANSI must be square. Check input.shape.raw or input.shape.file");
  process.exit(3);
}
...
pub const DEBUG: bool = ${debug===null?config.debug:debug};
...
pub const START_POSITION_Y: i16 = ${shape.start_position.y};
...
pub const BEFORE_INPUT_START: &'static str = "${makeSafety(shape.before_start??"")}";

pub const PASSWORD_CHAR: char = '${makeSafetyChar(shape.password_char??'*')}';
